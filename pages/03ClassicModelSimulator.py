...
from inventory_models.demand_models import Poisson_Demand, Normal_Demand
...
demand_dist_type = st.selectbox(label='Select the probability density distribution', options=['Normal', 'Poisson'])

if demand_dist_type == 'Normal':
...
with reorder_parameters_column:
    ...
    if "eoq_value" in st.session_state:
        eoq_value = st.session_state["eoq_value"]
        quantity_to_order = st.number_input(label="What is the quantity to order when get the reorder point?", min_value=0.0, value=eoq_value)
    else:    
        quantity_to_order = st.number_input(label="What is the quantity to order when get the reorder point?", min_value=0.0, value=400.0)
...
if submit:
    ...
    if demand_dist_type == 'Normal':
        demand_dist = Normal_Demand(mean=demand_mean, std_dev=demand_std)
    else:
        demand_dist = Poisson_Demand(mean=demand_mean)    
